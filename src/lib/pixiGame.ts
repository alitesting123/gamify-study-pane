...
/**
 * Initialize the PixiJS plane game
 */
...
async function startPlaneGame() {
  ...
  function checkAnswer() {
    ...
    if (correct) {
      ...
      gameState.callbacks.onQuestionComplete?.(true);
      ...
      userAnswer = '';
      quizAnswerText.text = '';
      gameState.callbacks.onQuestionComplete?.(false);
  ...
  app.ticker.add((ticker) => {
    ...
    debris = debris.filter(d => {
      ...
      if (!gameState.invincible && d.checkCollision(plane.x, plane.y)) {
        ...
        if (gameState.planeHealth <= 0) {
          ...
          gameState.callbacks.onGameComplete?.(gameState.debrisDestroyed * 10);
        }
        d.remove();
        return false;
      }
    ...
    gameState.callbacks.onScoreUpdate?.(gameState.debrisDestroyed * 10, gameState.distance);
  });
}
